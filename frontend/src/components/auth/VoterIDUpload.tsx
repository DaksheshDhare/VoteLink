...
import { User, Loader2, AlertCircle, Shield, CheckCircle2, Upload, ArrowLeft, XCircle } from 'lucide-react';
...
export const VoterIDUpload: React.FC<VoterIDUploadProps> = ({
  ...
  const verifyFaceInVoterID = async (file: File) => {
    ...
    try {
      ...
      // More lenient validation - allow manual entry if OCR fails
      if (!extractedInfo.voterID || extractedInfo.voterID.length < 4) {
        console.warn('⚠️ Could not extract Voter ID from card automatically.');
        setError('⚠️ Could not extract Voter ID automatically. Please enter it manually below and ensure your photo is clear.');
        setOcrProgress('');
        // Don't remove the image - allow manual input
        setVerifyLoading(false);
        return;
      }
      ...
      console.log('   Voter ID:', extractedInfo.voterID);
      
      // Skip face detection on voter ID card for speed - face verification happens during live capture
      setOcrProgress('Verifying document...');
      
      // Brief delay for UI feedback
      await new Promise(resolve => setTimeout(resolve, 300));
      
      console.log('✅ Voter ID document verified!');
      
      // All verifications passed - auto-fill the form
      setVoterID(extractedInfo.voterID);
      setExtractedData({
        name: extractedInfo.name,
        voterID: extractedInfo.voterID
      });
        ...
        <div className="text-center mb-4">
          ...
          <p className="text-xs text-gray-700">Enter your voter ID and upload your Voter ID photo</p>
            ...
            <div className="space-y-3">
              ...
              {faceMatchResult && (
                <div className={`border-2 rounded-lg p-3 flex items-start space-x-2 shadow-lg ${
                  ...
                  <div>
                    <p className={`text-sm font-bold ${
                      faceMatchResult.matched ? 'text-green-800' : 'text-red-800'
                    }`}>
                      {faceMatchResult.matched ? '✅ Face Verified!' : '❌ Face Mismatch'}
                    </p>
                    <p className={`text-xs ${
                      faceMatchResult.matched ? 'text-green-700' : 'text-red-700'
                    }`}>
                      {faceMatchResult.message}
                    </p>
                    <p className={`text-xs mt-1 ${
                      faceMatchResult.matched ? 'text-green-600' : 'text-red-600'
                    }`}>
              ...
              {faceVerificationInProgress && (
                <div className="bg-blue-100 border-2 border-blue-500 rounded-lg p-3 flex items-center space-x-2">
                  <Loader2 className="text-blue-600 animate-spin" size={20} />
                  <p className="text-blue-800 text-sm font-bold">Verifying face match...</p>
                </div>
              )}

              {error && !faceMatchResult && (
                <div className="bg-red-100 border-2 border-red-500 rounded-lg p-2.5 flex items-start space-x-2 shadow-lg">
              ...
              {ocrProgress && (
                <div className="bg-blue-100 border-2 border-blue-500 rounded-lg p-2.5 flex items-start space-x-2">
                  <Loader2 className="text-blue-600 flex-shrink-0 animate-spin" size={18} />
                  <p className="text-blue-800 text-xs font-bold">{ocrProgress}</p>
                </div>
              )}
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-black/80 text-xs font-medium mb-1.5">
                    Voter ID Number
                  </label>
                  <input
                    type="text"
